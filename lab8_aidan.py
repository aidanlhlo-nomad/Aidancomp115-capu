...
def remove_duplicates_set(s):
    """
    This function removes the duplicates in the data set from string s 
    """
    seen_set=set(s)
    for c in s:
        if c not in s:
...
def gem_counting(stones, gems):
    """
    This function counts how many gems are in stones
    """
    count=0
    gem_set=set(gems)
    for c in stones:
...
# Your unit tests
assert gem_counting("cdEGHJ","cDEgHJ")== 4 
assert gem_counting("fgHIJK","FGhijk")== 0
...
def students_id(ids):
    """
    This function counts the number of distinct students ids when using shuttle bus in the list 
    """
    unique_ids=set(ids) 
    count=0
...
def students_id_occurrences(ids):
    """
    This function measure the occurences of Student ID who uses the shuttle bus service in the list
    """
    id_dict={}
...
def word_frequency(paragraph):
    """
    This function coutns the frequency of words in paragraphs
    """
    paragraph_dict={}
...
def id_name_repo_starred_300k(response_dict):
    """
    This function returnsa dictionary of all repository id in name pairs 
