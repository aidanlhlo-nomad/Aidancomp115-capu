...
def draw_circles4(x, size,shrink):
    for _ in range(4):
        x.circle(size)
        size = size - shrink

draw_circles4(x,55,14)
...
def draw_special4(x, size, shrink, replicate):
    for _ in range(replicate):
        draw_circles4(x, size,shrink)
        x.right(360 / replicate)

draw_special4(x,55,14,5)
...
def draw_picture_nice():
    ...
    for i in range(len(colors)):
        x.color(colors[i])
        draw_special4(x,size,decrease_amounts, replicate )
